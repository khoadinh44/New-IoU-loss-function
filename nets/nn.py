...
def conv(x, filters, k=1, s=1):
    if s == 2:
        ...
        padding = 'same'
    x = layers.Conv2D(filters, k, s, padding, use_bias=False, kernel_initializer=initializer, kernel_regularizer=l2)(x)
...
def box_iou(pred_boxes, valid_true_boxes, DIoU=False, CIoU=True, GIoU=False):
    ...
    pred_x1, pred_y1 = pred_box_xy[..., 0] - pred_box_wh[..., 0] / 2., pred_box_xy[..., 1] - pred_box_wh[..., 1] / 2.
    pred_x2, pred_y2 = pred_box_xy[..., 0] + pred_box_wh[..., 0] / 2., pred_box_xy[..., 1] + pred_box_wh[..., 1] / 2.
    
    true_x1, true_y1 =  true_box_xy[..., 0] - true_box_wh[..., 0] / 2., true_box_xy[..., 1] - true_box_wh[..., 1] / 2.
    true_x2, true_y2 = true_box_xy[...,0] + true_box_wh[..., 0] / 2., true_box_xy[..., 1] + true_box_wh[..., 1] / 2.
    ...
    if CIoU == True:
        cw = tf.math.maximum(pred_x2, true_x2) - tf.math.minimum(pred_x1, true_x1)  # convex (smallest enclosing box) width
        ch = tf.math.maximum(pred_y2, true_y2) - tf.math.minimum(pred_y1, true_y1)  # convex height
        ...
        if DIoU or CIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            rho2 = ((true_x1 + true_x2) - (pred_x1 + pred_x2)) ** 2 / 4 + ((true_y1 + true_y2) - (pred_y1 + pred_y2)) ** 2 / 4
            if DIoU:
                return iou - rho2 / c2  # DIoU
            elif CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * tf.math.pow(tf.math.atan2(true_box_wh[..., 0], true_box_wh[..., 1]) \
                                                - tf.math.atan2(pred_box_wh[..., 0], pred_box_wh[..., 1]), 2)
                # with torch.no_grad():
                alpha = v / (1 - iou + v)
